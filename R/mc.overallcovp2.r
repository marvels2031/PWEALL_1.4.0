#' Calculate the utility function p2 for NPH MCP-Mod covariance
#'
#' This function is to calculate the utility function p2 for the covariance of max-combo tests.
#' 
...
#' 
#' @details
#' This function is to calculate as covbeta2[j,j']
#' \deqn{\int_0^{t'} w_j(s)w_{j'}(s)G_E(t'-s)\{\pi_1q^2_0(s)\zeta_1(s)+\pi_0q^2_1(s)\zeta_0(s)\}ds;}
#' calculate as covbeta3[j,j']
#' \deqn{\int_0^t \int_0^{t'} I(s'\le s)w_j(s)w_{j'}(s')I(t-s> t'-s')G_E\{(t-s)\wedge (t'-s')\}\Big\{\frac{q_1(s')}{r(s')}\zeta(s')\Big\}q_1(s)a(s)ds'ds}
#' \deqn{-\int_0^t \int_0^{t'} I(s'\le s) w_j(s)w_{j'}(s')I(t-s> t'-s')G_E\{(t-s)\wedge (t'-s')\}\Big\{\frac{q_0(s')}{r(s')}\zeta(s')\Big\}q_0(s)a(s)ds'ds}
#' \deqn{=\int_0^{t'} w_{j'}(s')G_E(t'-s')\Big\{\frac{q_1(s')}{r(s')}\zeta(s')\Big\}\Big\{\int_{s'}^{s'+(t-t')}w_{j}(s)q_1(s)a(s)ds\Big\}ds'}
#' \deqn{-\int_0^{t'} w_{j'}(s')G_E(t'-s')\Big\{\frac{q_0(s')}{r(s')}\zeta(s')\Big\}\Big\{\int_{s'}^{s'+(t-t')}w_{j}(s)q_0(s)a(s)ds\Big\}ds';}
#' calculate as covbeta4[j,j']
#' \deqn{\int_0^t \int_0^{t'} I(s\le s')w_j(s)w_{j'}(s')G_E\{(t-s)\wedge (t'-s')\}\Big\{\frac{q_1(s)}{r(s)}\zeta(s)\Big\}q_1(s')a(s')ds'ds}
#' \deqn{-\int_0^t \int_0^{t'} I(s\le s')w_j(s)w_{j'}(s')G_E\{(t-s)\wedge (t'-s')\}\Big\{\frac{q_0(s)}{r(s)}\zeta(s)\Big\}q_0(s')a(s')ds'ds}
#' \deqn{=\int_0^{t'} w_{j'}(s')G_E(t'-s')q_1(s')a(s') \Big\{\int_0^{s'} w_{j}(s)\frac{q_1(s)}{r(s)}a(s)ds\Big\}ds'}
#' \deqn{-\int_0^{t'} w_{j'}(s')G_E(t'-s')q_0(s')a(s') \Big\{\int_0^{s'} w_{j}(s)\frac{q_0(s)}{r(s)}a(s)ds\Big\}ds';}
#' and calculate as EA2[j']
#' \deqn{\int_0^{t'} w_{j'}(s')G_E(t'-s')a(s')ds}
#' 
...
#' mc.overallcovp2(wfunctions=c(1,2),wfunctions0=c(1,2),tfix = 2, tfix0 = 1)
#' 
#' @export
#'
mc.overallcovp2=function(wfunctions=c(1,2),wfunctions0=c(1,2),tfix = 2, tfix0 = 1, 
                         ...
                         veps = 0.01, beta = 0, beta0 = 0) 
{
    ...
    covbeta2 <- covbeta3 <- covbeta4 <- matrix(0,nrow=nw,ncol=nw0)
    EA2 <- rep(0,nw0)
    if (sum(atplus < (tfix0 - err/10)) > 0) {
        ats <- atplus[atplus < (tfix0 - err/10)]
        atsupp <- c(ats, tfix0)
        nsupp <- length(atsupp)
        BigK1 <- pwecxpwuforvar(tfix = tfix0, t = atsupp, taur = taur, 
        ...
        BigK0 <- pwecxpwuforvar(tfix = tfix0, t = atsupp, taur = taur, 
        ...
        q1bs0 <- aa10/r1bs0
        q0bs0 <- aa00/r0bs0
        ak1 <- mc.innervar(wfunctions=wfunctions,t = tk1, taur = taur, u = u, ut = ut, 
            pi1 = pi1, rate11 = rate11, rate21 = rate21, rate31 = rate31, 
            ...
            rp21 = rp21, rp20 = rp20, eps = eps, veps = veps, 
            beta = beta)
        ak0 <- mc.innervar(wfunctions=wfunctions,t = tk0, taur = taur, u = u, ut = ut, 
            pi1 = pi1, rate11 = rate11, rate21 = rate21, rate31 = rate31, 
            rate41 = rate41, rate51 = rate51, ratec1 = ratec1, 
            rate10 = rate10, rate20 = rate20, rate30 = rate30, 
            rate40 = rate40, rate50 = rate50, ratec0 = ratec0, 
            ...
            rp21 = rp21, rp20 = rp20, eps = eps, veps = veps, 
            beta = beta)
        bk1 <- mc.innercov(wfunctions=wfunctions,tupp = tk1 + (tfix - tfix0), tlow = tk1, 
            taur = taur, u = u, ut = ut, pi1 = pi1, rate11 = rate11, 
            rate21 = rate21, rate31 = rate31, rate41 = rate41, 
            rate51 = rate51, ratec1 = ratec1, rate10 = rate10, 
            rate20 = rate20, rate30 = rate30, rate40 = rate40, 
            rate50 = rate50, ratec0 = ratec0, tchange = tchange, 
            type1 = type1, type0 = type0, rp21 = rp21, rp20 = rp20, 
            eps = eps, veps = veps, beta = beta)
        bk0 <- mc.innercov(wfunctions=wfunctions,tupp = tk0 + (tfix - tfix0), tlow = tk0, 
            taur = taur, u = u, ut = ut, pi1 = pi1, rate11 = rate11, 
            rate21 = rate21, rate31 = rate31, rate41 = rate41, 
            rate51 = rate51, ratec1 = ratec1, rate10 = rate10, 
            rate20 = rate20, rate30 = rate30, rate40 = rate40, 
        ...
        for (j in 1:nw0){
            ...
            EA2[j] <- pi1 * sum((1 - q1bs0) * dk1*wf1j) - (1 - pi1) * sum(q0bs0 * 
            dk0*wf0j)
            for (i in 1:nw){
                ai=wfunctions[i]
                ...
                covbeta2[i,j] <- pi1 * sum((1 - q1bs) * (1 - q1bs0) * dk1*wf1i*wf1j) + 
            (1 - pi1) * sum(q0bs * q0bs0 * dk0*wf0i*wf0j)
        covbeta3[i,j] <- pi1 * sum(bk1$qf1[,i] * q1bs0/r1bs0 * dk1*wf1j) + 
            (1 - pi1) * sum(bk0$qf1[,i] * q0bs0/r0bs0 * dk0*wf0j)
        covbeta3[i,j] <- covbeta3[i,j] - exp(beta0) * pi1 * sum(bk1$qf2[,i] * 
            (1 - q1bs0)/r1bs0 * dk1*wf1j) - exp(beta0) * (1 - pi1) * 
            sum(bk0$qf2[,i] * (1 - q0bs0)/r0bs0 * dk0*wf0j)
        covbeta4[i,j] <- pi1 * sum(ak1$qf1[,i] * q1bs0 * (1 - q1bs0) * 
            dk1*wf1j) - (1 - pi1) * sum(ak0$qf1[,i] * q0bs0^2 * dk0*wf0j)
        covbeta4[i,j] <- covbeta4[i,j] - exp(beta) * pi1 * sum(ak1$qf2[,i] * 
            (1 - q1bs0)^2 * dk1*wf1j) + exp(beta) * (1 - pi1) * sum(ak0$qf2[,i] * 
            q0bs0 * (1 - q0bs0) * dk0*wf0j)
          }
        }
    }
    cov234 <- covbeta2 + covbeta3 + covbeta4
    list(cov234 = cov234, covbeta2 = covbeta2, covbeta3 = covbeta3, 
        covbeta4 = covbeta4, EA2 = EA2)
}
