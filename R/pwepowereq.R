############################################################################################################################
#   A function to calculate powers at different timepoints for the equivalence trials (based on hazard ratio)
#   account for delayed treatment, discontinued treatment and non-uniform entry
############################################################################################################################
pwepowereq<-function(t=seq(0.1,3,by=0.5),uppermargin=1.3,lowermargin=1/uppermargin, alpha=0.05,taur=1.2,u=c(1/taur,1/taur),ut=c(taur/2,taur),pi1=0.5,
  ...
  alpha1<-alpha
  aa<-qnorm(alpha1)/sqrt(avar1$xdenom*avar1$vbeta)+sqrt(n)*(log(uppermargin)-a1x$b1)/sqrt(avar1$vbeta)
  aa1<-qnorm(alpha1)+sqrt(n)*(log(uppermargin)-a1x$b1)/sqrt(avar1$vbeta)

  bb<-qnorm(1-alpha1)/sqrt(avar1$xdenom*avar1$vbeta)+sqrt(n)*(log(lowermargin)-a1x$b1)/sqrt(avar1$vbeta)
  bb1<-qnorm(1-alpha1)+sqrt(n)*(log(lowermargin)-a1x$b1)/sqrt(avar1$vbeta) ###assuming Fisher information=variance of beta

  coxpower<-coxpower1<-0
  if (bb<aa) coxpower<-pnorm(aa)-pnorm(bb)
  if (bb1<aa1) coxpower1<-pnorm(aa1)-pnorm(bb1)
